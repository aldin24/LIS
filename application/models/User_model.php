...
class User_model extends CI_Model {
	...
	public function issue_limit( $userid ) {
		$string = "SELECT book_limit FROM user, user_type WHERE user.type_id = user_type.id AND user.id = $userid";
		$query = $this->db->query( $string );
		$rows = $query->result_array();
		return $rows[0]['book_limit'];
	}
	...
	public function email_confirm( $id, $password ) {
